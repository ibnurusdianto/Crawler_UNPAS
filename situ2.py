...
def ensure_directory_exists(directory):
    if not os.path.exists(directory):
        ...
        logger.info(f"[green]Created directory {directory}[/green] 📁")
        ...
        if response.status_code == 200:
            ...
            if redirect_url:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"[yellow]Failed to download {filename} after redirect (status code: {response.status_code})[/yellow] ⚠️")
            else:
